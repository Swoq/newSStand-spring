...
class SubscriptionControllerTest {
    ...
    void getAccountPage() throws Exception {
        ...
        given(rateService.findById(rate.getRateId())).willReturn(Optional.of(rate));
    ...
    void shouldReturnNotFound() throws Exception {
        given(rateService.findById(any())).willReturn(Optional.empty());
    ...
    void hasAlreadySubscribed() throws Exception {
        given(userService.findById(user.getId())).willReturn(Optional.of(user));
        given(rateService.findById(any())).willReturn(Optional.of(this.rate));
    ...
    void shouldCancerSubscription() throws Exception {
        ...
        given(subscriptionService.findById(subscription.getSubscriptionId()))
    ...
    void shouldReturnNotFoundSubscription() throws Exception {
        given(subscriptionService.findById(any())).willReturn(Optional.empty());
        given(userService.findById(user.getId())).willReturn(Optional.of(user));
        MediaType textHtml = new MediaType(MediaType.TEXT_HTML, StandardCharsets.UTF_8);
        mockMvc.perform(post("/user/subscriptions/cancel")
                .param("subscriptionId", String.valueOf(subscription.getSubscriptionId())))
    ...
    void userUnBindWithSubscription() throws Exception {
        ...
        given(userService.findById(this.user.getId())).willReturn(Optional.of(this.user));
        given(subscriptionService.findById(any())).willReturn(Optional.of(subscription));
